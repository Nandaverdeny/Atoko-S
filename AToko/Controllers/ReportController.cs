...
namespace AToko.Controllers
{
    ...
    public class ReportController : Controller
    {
        ...
        public ActionResult StockPrediction()
        {
            ...
            return View(new ViewModel { listProduct = list});
        }
        ...
        public ActionResult StockPrediction(ViewModel obj)
        {
            ...
            if (listOrderByMonth.Count() > 0 && listSalesByMonth.Count() > 0 && listStocksByMonth.Count() > 0)
            {
                ...
                var lastSalesObj = listSalesByMonth.OrderByDescending(o => o.Month).ThenByDescending(o => o.Year).FirstOrDefault();
                int lastSales = lastSalesObj.Qty;

                int maxStock = listStocksByMonth.Max(o => o.Qty);
                int minStock = listStocksByMonth.Min(o => o.Qty);
                var lastStockObj = listStocksByMonth.OrderByDescending(o => o.Month).ThenByDescending(o => o.Year).FirstOrDefault();
                int lastStock = lastStockObj.Qty;

                int maxOrder = listOrderByMonth.Max(o => o.Qty);
                int minOrder = listOrderByMonth.Min(o => o.Qty);
                var lastOrderObj = listOrderByMonth.OrderByDescending(o => o.Month).ThenByDescending(o => o.Year).FirstOrDefault();
                ...
                var listOrder = listOrderByMonth.OrderBy(o => o.Month).ThenBy(o => o.Year);
                var listSales = listSalesByMonth.OrderBy(o => o.Month).ThenBy(o => o.Year).ToList();
            ...
            {
                ViewBag.Result = "Insufficient data, cannot calculating stock prediction";
            }
